...
fn main() {
    ...
    for chrom in common_chroms.iter() {
        ...
        let offset = min_val;
        for &(start, end) in intervals1.iter() {
            chrom_region[start - offset] += 1;
            chrom_region[end - offset] -= 1;
        }
        for &(start, end) in intervals2.iter() {
        ...
        results.insert(chrom.to_string(), intersect);
    }
...
fn read_bed(file_path: &str) -> BTreeMap<String, (usize, usize, Vec<(usize, usize)>)> {
    ...
    let mut regions = BTreeMap::<String, (usize, usize, Vec<(usize, usize)>)>::new();
    while reader.read_line(&mut line).unwrap() > 0 {
        {
            ...
            if regions.contains_key(&chrom) {
                let value = regions.get_mut(&chrom).unwrap();
                value.0 = min(value.0, start);
                value.1 = max(value.1, end);
                value.2.push((start, end));
            } else {
                regions.insert(chrom, (start, end, vec![(start, end)]));
            }
        }
        line.clear();
    }
